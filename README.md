...
- **Cloze deletion, invisibly.** Select text and press Alt+C (or the Cloze
  ...
  note, and becomes basic again when the last one goes — the server re-detects
  the type from content on every save. The back is disabled while a note is a
  cloze (the deletions are the answers) and comes back when the last one goes.
